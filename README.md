...
共通基盤の取り込みに加えて、**税の土台（税率マスタと商品への税率紐付け）**と、**CRM のデータ構造（4 テーブル・モデル・リレーション・ダミーデータ）**まで実装済みです。画面と金額計算はこれからです。
...
| 4 | 顧客・商談の画面（一覧 / 詳細 / 登録・編集） | 未着手 |
| 5 | 金額の自動計算（明細 → 商談合計）と売上ダッシュボード | 未着手 |
...
共通基盤としては**完成**しています。テスト 118 件・PHPStan level 5・Pint がすべて通る状態を維持しています（件数には CRM 側で追加したテストを含みます）。
...
| **品質** | Pint / Larastan(level 5) / PHPUnit 118 件 | GitHub Actions で自動実行 |
