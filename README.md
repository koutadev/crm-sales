...
| 商談一覧 | `/deals` | 絞り込み結果に連動する金額サマリ。ステータス・顧客・営業担当で絞り込み。CSV に消費税・税抜も出力 |
| 商談詳細 | `/deals/{id}` | 明細・金額内訳（税込／うち消費税／税抜）・**税率別の内訳**・活動履歴（インライン追加） |
...
| 商談一覧 | **8** | 件数 ＋ 本体 ＋ 関連の eager load 3 本 ＋ サマリ 1 本 ＋ 絞り込みの選択肢 2 本 |
| 商談詳細 | **9** | 商談・明細・商品・税率・活動などの eager load |
...
共通基盤としては**完成**しています。このリポジトリでは CRM を上に載せた状態で、テスト 160 件・PHPStan level 5・Pint がすべて通る状態を維持しています。
