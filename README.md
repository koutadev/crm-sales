...
共通基盤の取り込みに加えて、**税の土台**・**CRM のデータ構造**・**顧客管理**・**商談と明細（登録・一覧・詳細）／内税の金額計算**まで実装済みです。残るは売上ダッシュボードです。
...
共通基盤としては**完成**しています。テスト 154 件・PHPStan level 5・Pint がすべて通る状態を維持しています（件数には CRM 側で追加したテストを含みます）。
...
| **品質** | Pint / Larastan(level 5) / PHPUnit 154 件 | GitHub Actions で自動実行 |
