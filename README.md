...
基本設計書に沿って、**税の土台**・**CRM のデータ構造**・**顧客管理**・**商談と明細（登録・一覧・詳細）／内税の金額計算**・**売上ダッシュボード**まで実装済みです。
...
共通基盤としては**完成**しています。テスト 160 件・PHPStan level 5・Pint がすべて通る状態を維持しています（件数には CRM 側で追加したテストを含みます）。
...
| **商談の可視化** | 商談一覧（絞り込みに連動する金額サマリ）/ 商談詳細（税率別の内訳・活動履歴のインライン追加） | サマリは絞り込み後の全件を 1 クエリで集計。CSV には消費税・税抜も出力 |
| **売上ダッシュボード** | KPI（今月の受注 / 受注見込み / 進行中 / 受注残）・月次売上推移・担当者別売上・パイプライン | 受注日ベース・税込。**集計は 4 クエリ**（実測値をテストで固定） |
...
| **品質** | Pint / Larastan(level 5) / PHPUnit 160 件 | GitHub Actions で自動実行 |
