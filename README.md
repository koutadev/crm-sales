...
| 組織 | `/masters/organizations` | 地域 > エリア > 店舗 の 3 段。店舗は所在都道府県を持つ。社員の所属先で、売上の集計単位 |
| 売上目標 | `/masters/sales-targets` | 全社・地域・エリア・店舗・担当者ごとの月次目標。前の期間から複製できる |
...
| 画面 | クエリ本数 | 内訳 |
| --- | --- | --- |
| ダッシュボード | **7** | KPI 4 種 / 月次推移 / 担当者別 / パイプライン が各 1 本 ＋ 組織別 2 本（担当者ごとの受注 1 本 ＋ 組織の一覧 1 本）＋ 予実の目標 1 本 |
...
売上目標は独立したマスタ（`sales_targets`）で持ち、**商談・明細には一切手を入れていません**。
達成率は「実績（既存の受注集計）÷ 目標」で出します。

| 項目 | 内容 |
| --- | --- |
| 粒度 | 全社 / 地域 / エリア / 店舗 / 担当者（対象タイプ + 対象 ID） |
| 期間 | 年月（月次）。四半期・年度はその合計として扱います |
| 入力 | `/masters/sales-targets`。**「前の期間から複製」** で、ある年月の目標をまとめて別の年月へコピーできます（すでにある対象は上書きしません） |
| 表示 | ダッシュボードに 当月 / 今年度のゲージ、月次推移グラフの**目標ライン**（破線）、組織別・担当者別の達成率 |

読むクエリは **3 本**（目標 1 本 ＋ 担当者ごとの実績 1 本 ＋ 組織の一覧 1 本）。
実績のクエリは「累計 / 当月 / 年度」を条件付き集計で同時に出すので、期間を増やしても本数は変わりません。

### 組織別の売上（地域 > エリア > 店舗 > 担当者）

ダッシュボードに、組織階層で売上（受注・税込）を積み上げたツリー表があります。
行の `▸` で **地域 → エリア → 店舗 → 担当者** と掘り下げられます（既存の担当者別グラフはそのまま）。

集計の考え方は 3 つだけです。

1. **商談テーブルには組織を持たせない**。`deals.employee_id → employees.organization_id` とたどって積み上げます
   （組織の付け替えは担当者の所属を変えるだけで済み、伝票の再計算は起きません）
2. 読むクエリは **2 本**。「担当者ごとの受注金額・件数」を 1 本、「組織の一覧」を 1 本。
   あとは PHP 側で店舗 → エリア → 地域に足し上げます
3. 階層は 3 段に固定されているので**再帰は不要**。行数は「組織数 + 担当者数」で頭打ちになり、
   商談が何件あってもクエリ本数も処理量も増えません（`OrganizationSalesTest`）

**都道府県で束ねた切り口**にも切り替えられます（店舗の `prefecture` で集め直すだけなので、追加のクエリは要りません）。
階層を深くせずに「同じ都道府県の複数店舗をまとめて見る」ができます。

所属が未設定の担当者は「未所属」としてまとめ、合計には含めます。
ツリーの合計は、商談テーブルの受注合計と必ず一致します（金額の計算ロジックには触れていません）。
