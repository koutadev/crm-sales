...
| 商談一覧 | 商談を金額・確度・ステータスで絞り込み/並べ替え。**上部に絞り込み結果の合計金額サマリ（税込）** |
...
- **税抜金額 ＝ 税込金額 − 消費税額**
- **税込金額 ＝ 税込単価 × 数量**（明細）
- 計算は**明細単位**で行い、**端数はすべて切り捨て**で統一する。
- **商談の合計金額（`deals.amount_total`）＝ 明細の税込金額の合算**。消費税合計・税抜合計も、明細ごとに求めた値を合算する（合計額から逆算し直さない。明細と合計がずれるのを防ぐため）。

> 税込から逆算する方式は、端数処理のルールを固定しないと同じ入力でも金額がぶれる。「**明細単位・切り捨て**」の 2 点は実装前に固定し、以後変更しない（README にも明記する）。
...
- **商談明細 `deal_items`（過去データ保全の要）**：商品を明細に追加した時点の**税込単価（`unit_price`）と税率%（`tax_rate_percent`）をコピー保持**する。金額は `amount_incl_tax`（税込＝正）を持ち、`tax_amount`（消費税）・`amount_excl_tax`（税抜）は 6-1 の式で逆算した結果を保存する（表示のたびに再計算しない）。以後、商品マスタや税率マスタが変わっても、確定済み商談の金額は不変。
- **表示と集計**：一覧サマリ・ダッシュボード・顧客/商談の金額集計は、すべて**税込基準**。商談詳細だけは内訳として「**税込 / うち消費税 / 税抜**」を並べて表示する。
