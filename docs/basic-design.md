...
| 商談一覧 | **表 / カンバン（パイプライン）** を切替。商談を金額・確度・ステータス・**期間（予定クローズ日／受注日を切替）** で絞り込み/並べ替え。条件は記憶され、**保存ビュー（マイビュー）** で呼び出せる。**上部に絞り込み結果のサマリ（件数・税込合計・加重見込み・ステータス別内訳バー）** |
...
- 静的デモ（Vercel）は本設計の代表 6 画面（ダッシュボード / 商談一覧 / 商談カンバン / 商談詳細 / 顧客詳細 / マスタ管理）をテーマ適用して見せる。
  手書きではなく **本体の実画面 HTML を書き出してローカル化**する（`bin/export-static-demo.py`）。
  動作しない操作は「このデモでは動作しません」と明示し、常時バナーで架空のダミーデータであることを示す。
...
| 商談一覧の絞り込み連動サマリ（件数・金額・加重見込み・ステータス別内訳） | `App\Support\Crm\DealListSummary` ＋ `TableBuilder::filteredQuery()`（1 クエリ） |
...
- 静的デモの公開先： https://crm-demo-static-snowy.vercel.app （リポジトリは `crm-demo-static`）
